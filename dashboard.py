...
st.markdown('<style>div.block-container{padding-top:1rem;}<style>', unsafe_allow_html=True) # move the title up (positioning)

fl = st.file_uploader(":file_folder: Upload file", type=(["csv","txt","xlsx","xls"])) #uploading dataset
if fl is not None:
    filename = fl.name
    st.write(filename)
    df = pd.read_csv(filename, encoding="ISO-8859-1")
#else: 
#    os.chdir(r"D:\VS Code\sales-dashboard") # change directory
...
# creating a tree map based on region, category, sub-category
